...
import { FaTrash, FaPencilAlt, FaTimes, FaCheck, FaPlus } from 'react-icons/fa';
    ...
    return (
        <>
          <div className="relative h-24 w-full">
            <button onClick={() => setShowModal(true)} className="rounded-full bg-green-500 hover:bg-green-600 p-4 m-4 z-10 mt-7 absolute top-0 right-0">
            ...
            <table className="min-w-full">
              <thead className="sticky top-0 bg-white z-10">
                <tr className='bg-gray-200'>
                  ...
                  <th className="w-1/6 px-4 py-2">Team</th>
              ...
              <tbody>
                {users.map((user, index) => (
                  <tr key={user._id} className={`border-b ${editingIndex === index ? 'bg-gray-100 shadow-outline' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-6 py-4 whitespace-no-wrap text-sm leading-5 text-gray-900">
                      <span className="block w-full">{user.team}</span>
                    </td>
